...
COUNTY_ID  = "FIPS:41005"   # County code provided by NOAA (Clackamas, OR)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=f"Fetch NOAA temperature data for all weather stations in {COUNTY_ID}."
    )
    parser.add_argument(
        "--token", required=True,
    ...
    print(f"\nNOAA for {COUNTY_ID} — Temperature Data")
    ...
    temperatureDataFrame.to_csv('TestingAPITemperatureHood.csv', index=False)
